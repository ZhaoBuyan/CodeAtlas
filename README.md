...
来自小黑盒社区的一篇帖子：有人把《僵尸毁灭工程》的代码库做成了"代码图集"（仓库 `kolulu23/Zedema`）。
那个项目**没有 LICENSE**（无协议＝保留所有权利），所以这里**没有复制任何代码**，是另起炉灶的通用化实现；
当时和原作者在评论区交流过，对方表示"可以做"（通用化）。

如果你也享受"自己翻图去理解陌生代码"这个过程（而不是黑盒问答），那这个工具就是为这种需求做的。
