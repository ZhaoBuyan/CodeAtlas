...
| `types[]` | 名称、`fqn`、类别、命名空间、`dir`、`system` + `systemRule`（命中的分组规则）、**`doc`**（源码注释里的说明）、**`p` / `r`**（签名：参数表 / 返回类型，抽不到就没有这两个键）、文件 + 行号、LOC（该类型的区间）、成员统计与列表、基类、复杂度、fanIn / fanOut |
...
- **项目规则里写的**：`facets.json` 的 `exclude` 可以追加要忽略的目录（比如上游参考代码）；
  ...
  启动器上的 **「扫描范围」**（列表最后一项：**「按 .gitignore 跳过」**）或命令行 `--gitignore` 还会按项目自己的 `.gitignore` 跳过 ——
